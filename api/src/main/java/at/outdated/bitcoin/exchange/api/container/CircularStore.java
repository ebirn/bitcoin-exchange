 ...
 */
public class CircularStore<T> extends AbstractCollection<T> {


    /** Serialization version */
    private static final long serialVersionUID = 5603722883672945107L;

    /** Underlying storage array */
    private transient ArrayList<T> elements;
     ...
     */
    public CircularStore(Collection<T> coll) {
        this(coll.size());
        addAll(coll);
    }
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        elements = new ArrayList<T>(maxElements);
        int size = in.readInt();
     ...
     * @throws org.apache.commons.collections.BufferOverflowException  if this buffer is full
     */
    public boolean add(T element) {

        if (full) remove();

        elements.set(end++, element);
     ...
     * @throws org.apache.commons.collections.BufferUnderflowException  if the buffer is empty
     */
    public T get() {
    ...
    public T remove() {
        ...
        T element = elements.get(start);

        if (null != element) {
            elements.set(start++, null);
        ...
        return new Iterator() {
            ...
            public void remove() {
                ...
                if (start < lastReturnedIndex && pos < end) {
                    ...
                    while (pos != end) {
                        if (pos >= maxElements) {
                            elements.set(pos - 1, elements.get(0));
                            pos = 0;
                        } else {
                            elements.set(decrement(pos), elements.get(pos));
                            pos = increment(pos);
                        }
                    }
                }
